...
function main()
    ...
    if threadHandle then
        wait(50) -- Даем время потоку завершиться
    end
    ...
    -- Инициализируем время следующего действия
    nextActionTime = getTimeMs()
    
    -- Создаем новый поток
    threadRunning = true
    threadHandle = lua_thread.create(function()
        while threadRunning do
            wait(0)
            
            -- Проверяем доступность SAMP и игрока
            if not isSampAvailable() then
                wait(100)
                goto continue
            end
            
            -- Проверяем, что игрок существует и заспавнен
            if not sampIsLocalPlayerSpawned() then
                wait(100)
                goto continue
            end
            
            -- Проверяем, что PLAYER_PED валиден
            if not isCharOnFoot(PLAYER_PED) and not isCharInAnyCar(PLAYER_PED) then
                wait(100)
                goto continue
            end
            ...
            if config.enabled then
                ...
                if currentTime >= nextActionTime then
                    ...
                    if config.useCameraRotation then
                        if isCharOnFoot(PLAYER_PED) or isCharInAnyCar(PLAYER_PED) then
    ...
    imgui.OnFrame(function() return window.v end, function()
        ...
        -- Статус
        if config.enabled then
            local timeUntilNext = math.max(0, math.floor((nextActionTime - getTimeMs()) / 1000))
